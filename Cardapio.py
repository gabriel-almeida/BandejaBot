...
COMIDAS_IMPORTANTES = ["Kibe", "Quiche", "Almôndegas", "Peixe", "Estrogonofe", "Nhoque", "Farofa"]
...
class Cardapio():
    ...
    def __scrap_informacoes_cardapio(self, html):
        ...
        for id_dia_semana in range(len(DIAS_DA_SEMANA)):
            ...
            for id_refeicao in range(len(ORDEM_REFEICAO)):
                ...
                cardapio_coletado[dia_semana][refeicao] = self.__scrap_refeicao(table_rows, id_dia_semana, id_refeicao)
    ...
    def carrega_cardapio(self):
        ...
        try:
            ...
            logging.warning("Ocorreu um erro, agendando atualizacao agressiva. %s", str(e))
    ...
    def data_inicio_vigencia(self):
        ...
        """
        primeiro_dia_cardapio = self.data_cardapio - datetime.timedelta(self.data_cardapio.weekday())
    ...
    def compoe_mensagem(self, refeicao, dia_semana, dict_refeicao):
        ...
        msg = "O %s de %s (%s/%s) na UFRJ %s:\n" % (refeicao.lower(), dia_semana.lower(),
                                                data.day, data.month, VERBO_SER[self.__conjugacao_verbal(dia_semana)])
